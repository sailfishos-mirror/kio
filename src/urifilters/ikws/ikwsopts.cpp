    ...
    , m_settings(new WebShortcutsSettings(this))
{
    ...
    m_settings->setSharedConfig(KSharedConfig::openConfig(QString::fromUtf8(KURISearchFilterEngine::self()->name()) + QLatin1String("rc"),
                                                          KConfig::NoGlobals));
...
void FilterOptions::setDefaultEngine(const QString &engine)
{
    ...
    if (!engine.isEmpty()) {
        const auto matchIndexList = proxy->match(proxy->index(0, 0), ProvidersListModel::ShortNameRole,
                                                 engine, 1, Qt::MatchFixedString);
