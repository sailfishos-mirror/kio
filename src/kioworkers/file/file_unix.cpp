...
// per-file counter. errno is left set on failure.
                   ...
                   DestDirGroup destDirGroup)
{
    ...
    // a non-root worker can still set the group before a (possibly denied) owner change. As with
    // copy(), preserving the owner is best-effort and just warns when not permitted.
    if (!destDirGroup.imposedByDir && st.st_gid != destDirGroup.createdGid && ::fchown(destFd, -1 /*keep user*/, st.st_gid) != 0) {
...
// preserve(sourceFd, destFd, st, destFreshlyCreated, destDirGroup) runs after every successful
// content copy, with both fds open, and holds a callable. The engine is agnostic to what it does;
// the worker injects one that mirrors copy()'s preservation. destFreshlyCreated lets it skip a
// redundant fchmod, and destDirGroup is the group open() already gave the new file (worked out
// once per destination directory, see below).
using PreserveFn = std::function<void(int sourceFd, int destFd, const struct stat &st, bool destFreshlyCreated, DestDirGroup destDirGroup)>;
...
// the shared directory prefix on each open. destDirGroup is the group a freshly created file
// already has in destDirFd (its parent directory), and where that group comes from.
...
private:
    ...
    bool ensureDestDir(const QByteArray &dir)
    {
        ...
        struct stat ds;
        const bool setgid = ::fstat(openedFd, &ds) == 0 && (ds.st_mode & S_ISGID);
        m_destDirGroup = setgid ? DestDirGroup{ds.st_gid, true} : DestDirGroup{m_egid, false};
        return true;
    }
...
WorkerResult FileProtocol::batchCopy(QDataStream &stream)
{
    ...
    // group a freshly created file already has is worked out per destination directory by the
    // engine and handed in.
    const mode_t umaskBits = currentUmask();
    const uid_t euid = ::geteuid();
    const gid_t egid = ::getegid();
    PreserveFn preserve = [umaskBits, euid, this](int sourceFd, int destFd, const struct stat &st, bool destFreshlyCreated, DestDirGroup destDirGroup) {
        preserveAttrs(sourceFd, destFd, st, destFreshlyCreated, umaskBits, euid, destDirGroup);
#if HAVE_SYS_XATTR_H || HAVE_SYS_EXTATTR_H
