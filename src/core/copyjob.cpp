...
// Whether one of these clones a particular file is still up to the filesystem: XFS only does it
// when it was made with reflink support, and ZFS only from the version that brought block cloning,
// so a refusal is expected and the contents are copied instead. Network filesystems that can clone
// (NFS from 4.2, SMB3) are missing on purpose, the batch never runs on a network destination.
static bool isReflinkCapableFs(const QString &mountType)
{
    return mountType == QLatin1String("btrfs") || mountType == QLatin1String("xfs") || mountType == QLatin1String("bcachefs")
        || mountType == QLatin1String("ocfs2") || mountType == QLatin1String("zfs");
}
...
bool CopyJobPrivate::tryBatchCopyFiles()
{
    ...
    q->connect(job, &KIO::TransferJob::data, q, [this, q](KJob *, const QByteArray &payload) {
        ...
        for (const qint32 idx : std::as_const(done)) {
            if (idx >= 0 && idx < files.size() && !m_batchReported.contains(idx)) {
        ...
        if (current >= 0 && current < files.size()) {
            const CopyInfo &ci = files.at(current);
