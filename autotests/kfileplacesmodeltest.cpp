...
void KFilePlacesModelTest::testPlaceCollapsedUrl_data()
{
    ...
    // Laves trailing slash intact.
    QTest::newRow("home/foo/") << QUrl::fromLocalFile(m_tmpHome.path() + "/foo/"_L1) << u"Home/foo/"_s;

    QTest::newRow("trash") << QUrl(u"trash://"_s) << u"Trash"_s; // two slashes.
    QTest::newRow("trash root") << QUrl(u"trash:///"_s) << u"Trash"_s; // three slashes.
    QTest::newRow("trash/foo") << QUrl(u"trash:///foo"_s) << u"Trash/foo"_s;
